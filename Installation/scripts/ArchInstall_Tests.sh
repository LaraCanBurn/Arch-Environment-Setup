...
# Versión 2.1 - Con test unitarios y backup a USB
# ==============================================
...
Para restaurar una cabecera LUKS:
...
Guarde este directorio en un lugar seguro!
EOF
...
function handle_reboot() {
    ...
    # Ofrecer opción de backup antes de reiniciar
...
function run_tests() {
    ...
    # Test 1: Verificar root
    check_root
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}[✓] Test 1: check_root pasado${NC}"
        ((passed++))
    else
        echo -e "${RED}[✗] Test 1: check_root fallado${NC}"
        ((failed++))
    fi
    ...
    detect_env
    if [ $? -eq 0 ] && [ -n "$ENV" ]; then
        echo -e "${GREEN}[✓] Test 2: detect_env pasado - Entorno: $ENV${NC}"
        ((passed++))
    else
        echo -e "${RED}[✗] Test 2: detect_env fallado${NC}"
        ((failed++))
    fi
    
    # Test 3: Detección de USB (solo verifica que la función se ejecuta)
    detect_usb_devices
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}[✓] Test 3: detect_usb_devices pasado${NC}"
        ((passed++))
    else
        echo -e "${YELLOW}[!] Test 3: detect_usb_devices sin USBs detectados (puede ser normal)${NC}"
        ((passed++))  # No consideramos esto un fallo
    fi
